...
// QueryUserPosts queries the userPosts edge of a User.
...
// QueryUserID queries the user_id edge of a UserPost.
func (c *UserPostClient) QueryUserID(up *UserPost) *UserQuery {
...
func (c *UserPostClient) mutate(ctx context.Context, m *UserPostMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("ent: unknown UserPost mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		User, UserPost []ent.Hook
	}
	inters struct {
		User, UserPost []ent.Interceptor
	}
)
